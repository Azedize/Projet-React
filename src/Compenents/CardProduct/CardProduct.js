...
import { faBed, faBath, faToilet, faDoorClosed } from '@fortawesome/free-solid-svg-icons';
...
function DarkVariantExample() {
    ...
    console.log("Valeur de produit.date[0]:", produit.date);

    if (!produit) {
        return <div>Loading...</div>;
    }
    const alsetforRes = (cat,id,Nom) => {
        let timerInterval;

        const produitRes = cards[cat]?.find(el => el.id === id);

        dispatch({ type:'ADD_RESERVATION',
        payload:{
            id: uuidv4(),
            title: produitRes.title,
            avis:produitRes.avis,
            prix: produitRes.prix,
            nom:Nom,
            email: user.email
        }
    })
        ...
        Swal.fire({
            ...
            iconColor: "#86b7fe", 
            html: "Sauvgarde de Demande <b></b> milliseconds.",
            timer: 4000,
            ...
            if (result.dismiss === Swal.DismissReason.timer) {
                console.log("Sauvgarde de Demande");
            }
            ...
            Swal.fire({
                ...
                icon: "success",
                iconColor: "#86b7fe", 
                title: "Your Demande has been saved",
    ...
    return (
        <>
        <Carousel data-bs-theme="dark " className='corusol'>
            {produit.images.map((image, index) => (
                <Carousel.Item>
                    <div
                        style={{ backgroundImage: `url(${image})` }}
                        className="d-block w-100 img-curasol"
                    ></div>
                    <Carousel.Caption>
                        <h5 className='leui'>{produit.lieu}</h5>
                        <p className='title'>{produit.title}</p>
                    </Carousel.Caption>
                </Carousel.Item>
            ))}
        </Carousel>
        <div className='corusols'>
                <div className='corusol2'>
                    <div className='corusol3' style={{ marginTop: '15px' }}>
                    ...
                    <div className='corusol4'>
                        <div style={{textAlign:'center'}}>
                            {produit.avis}
                        </div>
                        <div className='corusol3'>
                    ...
                    <div className='corusol4' style={{ marginTop: '15px' }}>
                        <FaUserCircle style={{ fontSize: '24px',marginRight:'5px' }} />{produit.loyer}                    
                    </div>
                </div>
                <div className='corusol2'>

                    <div className='coruso'>
                        <div className='d-flex mtcd'>
                            <FontAwesomeIcon className='icons' icon={faDoorClosed} />
                            <p className='iconp'>Voyageurs : {produit.forme.voyageurs}</p>
                        </div>
                            <div className='d-flex mtcd'>
                            <FontAwesomeIcon className='icons' icon={faBed} />
                            <p className='iconp'>Chambres : {produit.forme.chambres}</p>
                        </div >
                            <div className='d-flex mtcd'>
                            <FontAwesomeIcon className='icons' icon={faBed} />
                            <p className='iconp'>Lits : {produit.forme.lits}</p>
                        </div>
                            <div className='d-flex mtcd'>
                            <FontAwesomeIcon className='icons' icon={faBath} />
                            <p className='iconp'>Salles De Bain : {produit.forme['salles de bain']}</p>
                        </div>
                            <div className='d-flex mtcd'>
                            <FontAwesomeIcon className='icons' icon={faToilet} />
                            <p className='iconp'>Toilette : {produit.forme.toilette}</p>
                        </div>
                    </div>
        ...
        <div className='corusol10'>
            <div style={{width:'40%'}} className='w100'>
                <h5>{Object.keys(produit.description)[0]}</h5>
                <div className='child'>
                    {
                        produit.description?.["Vues panoramiques"]?.map((el,index) => {
                            if (el.type) {
                                return (
                                    <div key={index} className='desciption'>
                                        <span>{el.icon}</span> {el.value} 
                                    </div>
                                );
                            }
                            return null;
                ...
                <h5>{Object.keys(produit.description)[1]}</h5>
                        <div className='child '>
                    {
                        produit.description?.["Chambre et linge"]?.map((el,index) => {
                            if (el.type) {
                                return (
                                    <div key={index} className='desciption'>
                                        <span>{el.icon}</span> {el.value} 
                ...
                <h5>{Object.keys(produit.description)[2]}</h5>
                        <div className='child ' >
                    {
                        produit.description?.["Divertissement"]?.map((el,index) => {
                            if (el.type) {
                                return (
                                    <div key={index} className='desciption'>
                                        <span>{el.icon}</span>{el.value} 
                    ...
                    <h5>{Object.keys(produit.description)[3]}</h5>
                        <div className='child ' >
                        {
                            produit.description?.["Famille"]?.map((el, index) => {
                                if (el.type) {
                                    return (
                                        <div key={index} className='desciption'>
                                            <span>{el.icon}</span> {el.value}
                    ...
                    <h5>{Object.keys(produit.description)[4]}</h5>
                        <div className='child '>
                        {
                            produit.description?.["Chauffage et climatisation"]?.map((el, index) => {
                                if (el.type) {
                                    return (
                                        <div key={index} className='desciption'>
                                            <span>{el.icon}</span> {el.value}
                ...
                <h5>{Object.keys(produit.description)[5]}</h5>
                    <div className='child'>
                    {
                        produit.description?.["Sécurité à la maison"]?.map((el,index) => {
                            if (el.type) {
                                return (
                                    <div key={index} className='desciption'>
                                        <span>{el.icon}</span> {el.value} 
                ...
                <h5>{Object.keys(produit.description)[6]}</h5>
                    <div className='child' >
                    {
                        produit.description?.["Internet et bureau"]?.map((el,index) => {
                            if (el.type) {
                                return (
                                    <div key={index} className='desciption'>
                                        <span>{el.icon}</span> {el.value} 
                ...
                <h5>{Object.keys(produit.description)[7]}</h5>
                    <div className='child'>
                    {
                        produit.description?.["Caractéristiques de l'emplacement"]?.map((el,index) => {
                            if (el.type) {
                                return (
                                    <div key={index} className='desciption'>
                                        <span>{el.icon}</span> {el.value} 
                ...
                <h5>{Object.keys(produit.description)[8]}</h5>
                    <div className='child'>
                    {
                        produit.description?.["Extérieur"]?.map((el,index) => {
                            if (el.type) {
                                return (
                                    <div key={index} className='desciption'>
                                        <span>{el.icon}</span> {el.value} 
                ...
                <h5>{Object.keys(produit.description)[9]}</h5>
                <div className='child' >
                    {
                        produit.description?.["Parking et installations"]?.map((el,index) => {
                            if (el.type) {
                                return (
                                    <div key={index} className='desciption'>
                                        <span>{el.icon}</span> {el.value} 
            ...
            <div className='corusol1'>
                    ...
                    title="UniqueTitle"
                        ...
                        height="481"
                        id="gmap_canvas"
                        src={`https://maps.google.com/maps?q=${produit.lieu}+&t=k&z=6&ie=UTF8&iwloc=&output=embed`}
                        frameBorder="0"
                        scrolling="no"
                        marginHeight="0"
                        marginWidth="0"
            ...
            <div className="fixed-bottom">
                    <div className="card-text-Price-p"><span>{produit.prix}$</span><span className='nuit'>par nuit</span></div>
                    <div className="card-text-date">
                        <div className='card-text-date-d'>
                            <span>Debut </span>
                            <div>{produit.date[0] ? produit.date[0].toLocaleDateString('en-GB') : 'Date non définie'}</div>
                        </div>
                        <div className='card-text-date-d'>
                            <span >Fin</span>
                            <div>{produit.date[1] ? produit.date[1].toLocaleDateString('en-GB') : 'Date non définie'}</div>
                        </div>
                    </div>
                    <div className="card-text-date-v">
                        <div className='card-text-date-vv'>
                            voyageurs : {produit.forme.voyageurs}
                        </div>
                    </div>
                    <div className="card-text-date-v">
                        <div className='card-text-date-vv'>
                            Prix Total : {dateFns.differenceInDays(produit.date[1], produit.date[0]) * produit.prix} $
                        </div>
                    </div>
                    <div class="bubbly-button-div-b">
                        <button class="bubbly-button-b" onClick={() => alsetforRes(cat, produit.id, user.nom )}>Envoyer Demande</button>
                    </div>
            </div>
